...
import { updateScore, getScore } from "../../../services/score";
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (score) {
        return {
          ...score,
          genres: score?.genres?.map((g) => g.id) || [],
          playedPlatforms: score?.playedPlatforms?.map((p) => p.id) || [],
        };
      }
      return score;
    },
        ...
        <Grid size={{ xs: 12, sm: 10, md: 8, lg: 6 }}>
          ...
          !platforms ? (
            ...
            <Paper elevation={0} style={{ padding: 20 }}>
              <Typography variant="h3">Edit {score?.name}</Typography>
              <Formik
                initialValues={score}
                validationSchema={createScoreSchema}
                onSubmit={async (values) => {
                  const formattedScore = formatScoreToUpdate(values);
                  await updateScore(formattedScore);
                  navigate(`/score/thoughts/${scoreId}`);
                }}
              >
                {({
                  values,
